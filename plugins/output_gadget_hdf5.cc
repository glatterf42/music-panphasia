...
#define GAS_PARTTYPE 2
#define HIGHRES_DM_PARTTYPE 0 //This is only true for PBH simulations b/c we hijacked the usual gastype!
#define COARSE_DM_DEFAULT_PARTTYPE 1
...
public:
  gadget_hdf5_output_plugin(config_file &cf) : output_plugin(cf) {
    ...
    coarsePartType = cf.getValueSafe<unsigned>("output", "gadget_hdf5_coarsetype", COARSE_DM_DEFAULT_PARTTYPE);
    UnitLength_in_cm = cf.getValueSafe<double>("output", "gadget_hdf5_unitlength", 3.085678e24); // 1.0 Mpc
    UnitMass_in_g = cf.getValueSafe<double>("output", "gadget_hdf5_unitmass", 1.989e43);         // 1.0e10 solar masses
    UnitVelocity_in_cm_per_s = cf.getValueSafe<double>("output", "gadget_hdf5_unitvel", 1e5);    // 1 km/sec
    ...
    useLongIDs = cf.getValueSafe<bool>("output", "gadget_hdf5_longids", false);
    numFiles = cf.getValueSafe<unsigned>("output", "gadget_hdf5_num_files", 1);
    doublePrec = cf.getValueSafe<bool>("output", "gadget_hdf5_doubleprec", 0);
    ...
    if (coarsePartType == GAS_PARTTYPE || coarsePartType == HIGHRES_DM_PARTTYPE)
      throw std::runtime_error("Error: Specified illegal Gadget particle type for coarse particles.");
  ...
  void finalize(void) {
    ...
    std::cout << " - GADGET HDF5 : wrote " << nPartTotAllTypes << " particles..." << std::endl;
    for (size_t i = 0; i < nPartTotal.size(); i++)
    ...
    for (unsigned i = 0; i < numFiles; i++) {
      ...
      // Write UNITS header using the physical constants assumed internally by Gadget(4)
    ...
    if (useLongIDs)
      std::cout << " - GADGET HDF5: Wrote 64bit IDs, enable LONGIDS." << std::endl;
    if (doublePrec)
      std::cout << " - GADGET HDF5: Double precision ICs, set INPUT_IN_DOUBLEPRECISION." << std::endl;
    if (NTYPES != 6)
      std::cout << " - GADGET HDF5: Using [" << NTYPES << "] particle types, set NTYPES to match." << std::endl;
    if (doBaryons)
      std::cout << " - GADGET HDF5: Wrote high-res gas (only), set REFINEMENT_HIGH_RES_GAS and GENERATE_GAS_IN_ICS with "
                << "SPLIT_PARTICLE_TYPE=" << pow(2, coarsePartType) << "." << std::endl;
    if (levelmax_ != levelmin_)
      std::cout << " - GADGET HDF5: Have zoom type ICs, set PLACEHIGHRESREGION=" << pow(2, HIGHRES_DM_PARTTYPE)
                << " (suggest PMGRID=" << pmgrid << " with GRIDBOOST=" << gridboost << ")." << std::endl;
    else
      std::cout << " - GADGET HDF5: Have unigrid type ICs (suggest PMGRID=" << pmgrid << ")." << std::endl;
    if (levelmax_ > levelmin_ + 1)
      std::cout << " - GADGET HDF5: More than one coarse DM mass using same type, set INDIVIDUAL_GRAVITY_SOFTENING="
                << pow(2, coarsePartType) << " (+" << pow(2, STAR_PARTTYPE) << " if including stars)." << std::endl;
    if (doBaryons)
      std::cout << " - GADGET HDF5: Set initial gas temperature to " << std::fixed << std::setprecision(3) << Tini << " K."
                << std::endl;
    std::cout << " - GADGET HDF5: Suggest grav softening = " << std::setprecision(3) << softening << " for high res DM."
              << std::endl;
  }
};

namespace {
output_plugin_creator_concrete<gadget_hdf5_output_plugin> creator("gadget_hdf5");
}
