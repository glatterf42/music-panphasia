...
template <typename T> inline T bytereorder(T v) {
  T rval;
...
template <typename T_store = float> class art_output_plugin : public output_plugin {
public:
  bool do_baryons_;
...
protected:
  enum iofields { id_dm_pos, id_dm_vel, id_gas_pos, id_gas_vel };
  ...
  public:
    pistream(std::string fname, size_t npart) : std::ifstream(fname.c_str(), std::ios::binary) {
      size_t blk;
...
public:
  explicit art_output_plugin(config_file &cf) : output_plugin(cf) {
    ...
    for (int i = 0; i < header_.nspecies; i++) {
      header_.wpart[i] =
          1.0 /
          pow(8.0, (header_.nspecies - i - 1)); // from high res to lo res // 8 should be changed for internal variable?
    }
